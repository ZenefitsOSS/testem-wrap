...
Bridge.prototype = new (function () {
  ...
  // Set up the channels in the start method which will be passed in from
  // runner script
  this.start = function (inChannel, outChannel) {
  ...
  this.acceptRequest = function (req, resp) {
    console.error('Accepted request');
    var self = this;
  ...
  this.handleRequest = function  (req, resp) {
    ...
    this.outClient.publish(channelOut, JSON.stringify(message));
  };
  ...
  this.sendCmd = function (message) {
    console.error('Sending command: ', message);
    this.outClient.publish(channelOut, JSON.stringify({
      type: 'cmd',
      data: message
    }));
  };
